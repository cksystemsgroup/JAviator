...
static volatile int8_t      converting;
...
{
    /* check for registered channels and that
       we're not converting data currently */
    if( selected_channel == -1 || converting )
    {
        return( -1 );
    }
...
/* Returns 1 if the ADC is idle, 0 otherwise.
*/
int8_t adc_is_idle( void )
{
    return( converting );
}
